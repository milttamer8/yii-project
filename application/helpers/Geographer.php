...
class Geographer extends Component
{
    ...
    public function geocodeQuery($query)
    {
        try {
            ...
            $provider = new Ipstack($httpClient, $this->ipStackApiKey);
            $cacheAdapter = new Adapter();
            $cacheProvider = new ProviderCache($provider, $cacheAdapter, 2628000);
            $geocoder = new StatefulGeocoder($cacheProvider, 'en');

            return $geocoder->geocodeQuery(GeocodeQuery::create($query));
